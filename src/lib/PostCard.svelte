...
<article class="mb-12">
  ...
  <div class="flex flex-col md:flex-row w-full lg:w-10/12">
    ...
    <div class="flex-1">
      <div class="text-sm font-medium text-gray-700">{new Date(post.publishedAt).toLocaleDateString('sv', {
        month: 'long',
        day: '2-digit',
        year: 'numeric'
      })}</div>
      ...
      <div class="mb-2">
        {#each post.categories as category}
          <a href="#" class="p-1 px-3 mr-1 mb-1 inline-block text-xs font-mono rounded bg-green-200 text-green-800 hover:bg-blue-200 hover:text-blue-800 transition duration-300 ease-in-out">
            {category.title}
          </a>
        {/each}
      </div>
